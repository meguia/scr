...
void processCommand() {
  ...
  if (processedcommand[1] == 'm') {
    ...
    for (int i = 2; i < sizeof(processedcommand); i+=4) {
      leds[(int)(i-2)/4].set(processedcommand[i], processedcommand[i+1], processedcommand[i+2], processedcommand[i+3]);
      //sprintf(msg, " setting: %i-%i-%i in %i time %i", processedcommand[i], processedcommand[i+1], processedcommand[i+2], (int)(i-2)/4, processedcommand[i+3]);
      //Serial.println(msg);
    }
    return;
